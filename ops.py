...
def deconv(x, feature=None, channels=128, kernel=4, stride=2, padding='SAME', use_bias=True, sn=True, scope='deconv'):
    with tf.variable_scope(scope):
        x_shape = feature.get_shape().as_list()
        print('feature shape:', x_shape)
        output_shape = tf.shape(feature[0, :, :, 0])
...
def expand_concat(x, z):
    ...
    z = tf.tile(z, multiples=tf.shape(x[0:1, :, :, 0:1]))  # expand#[1, x.shape[1], x.shape[2], 1]
    x = tf.concat([x, z], axis=-1)
